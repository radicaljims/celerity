...
-- jfs: Our model is just a list of toasts. Think we need anything more complex?
type alias Model = List Toast

-- jfs: The initial state of our application - no toasts!
emptyModel : Model
emptyModel = []
...
-- jfs: A little convenience to go with our 'Add Toast' button
fakeToast : Action
fakeToast = Add {kind = "nah", title = "greets", message = "hi there", id = -1, started = (0 * second)}

-- jfs: Another convenience, this time for our 'Remove Toast' button
popToast : Action
popToast = Pop

-- jfs: Updates the model (List Toast) based on the given action
update : Action -> Model -> Model
update action model =
  case action of
    -- jfs: As I indicated above, this handles timeout events.
    -- It doesn't do anything yet because I'm not sure how best to go
    -- about aging out toasts
    Tick -> model
...
-- map the renderToast method over the model!
view : Model -> Html Action
view model =
  div
    []
...
-- toasts ready to feed into our update() function.
